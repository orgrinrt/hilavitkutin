...
- SPLIT: **B2a** ships Resources + `Drop` (Column/Virtual nodes present as no-alloc placeholders); **B2b** ships Column buffers (needs `record_count`, which comes from the plan phase, so column allocation likely belongs in plan, not `build()` — settle in B2b).
