...
//! in the pipeline.
//!
//! §7-4a proved a facade registers + passes build()'s ContainsAll bound (no wall;
//! the sound facade declares only its bridge stores, the plugin's access is
//! non-host data). It called the opaque plugin symbol PER RECORD, which is faithful
//! for the build/DAG question but is the hot-path cost §7-4b must rule out. The
//! whole point of the plugin facade is that the cdylib boundary (an indirect call
//! the host optimiser cannot devirtualise) is crossed once per MORSEL BATCH, not
//! once per record, so the per-record hot path stays branch-and-pointer-free.
//!
//! Two things to prove, both in the real engine:
//!   1. HOST WALK ZERO BLR. A normal host fiber (Producer writes Column<Cv>,
//!      Consumer reads Cv + accumulates Sum) driven by the type-level RunFiberCol
//!      walk objdumps zero `blr` / zero `bl` EVEN WITH a facade WU in the same
//!      built pipeline. The facade's presence must not pull indirection into the
...
//! needs a `ctx.read_slice()/write_slice()` morsel-slice accessor that the per-WU
//! Context does not yet expose; that is a mechanical API addition, noted in the
//! outcome, not a feasibility question. Outcome recorded at the bottom.
...
// Proven type-level fiber walk (RunFiberCol, sketch 202606081200), reused verbatim.
...
// Host dispatch driver. Monomorphised per fiber type, so objdumping
// `dispatch_morsel_outer` gives one symbol per fiber: the HOST fiber's symbol must
// be zero blr; the FACADE fiber's symbol must show exactly the per-morsel ABI blr.
#[inline(never)]
...
impl WorkUnit<Always> for FacadePerMorsel {
    ...
    fn execute<'frame>(&self, ctx: &Self::Ctx<'frame>) {
        ...
        // The driver passes the absolute morsel start through MorselRange; the
        // facade needs absolute indices for the plugin, so it recovers start from
        // the batch range mapping. Here the plugin owns its data indexed by the
        // morsel-relative offset plus the driver-supplied base, so we hand it the
        // morsel-relative range; the call FREQUENCY (per morsel) is the claim.
...
fn main() {
    ...
    dispatch_morsel_outer(sched.__bindings(), &host_producer, USize(N), USize(MORSEL));
    let host_consumer = WuCons { head: ConsumerAccum, tail: WuNil };
    dispatch_unit_outer(sched.__bindings(), &host_consumer, USize(N));

    // Facade fiber: one ABI hop per morsel.
    let facade = WuCons { head: FacadePerMorsel, tail: WuNil };
    dispatch_morsel_outer(sched.__bindings(), &facade, USize(N), USize(MORSEL));
...
//
// Runtime: the host fiber (Producer -> ConsumerAccum) ran 256 records correct
// (Cv[i] = Sum[i] = i*PM), and the facade fired the opaque plugin capability
// exactly 8 times = once per morsel (MORSEL=32, ceil(256/32)=8), NOT 256 times.
// The plugin filled its own buffer correctly via its private cursor.
//
// objdump of the three isolated dispatch symbols:
//   HOST_producer_morsel : blr=0 bl=0 (81 instrs). The host producer walk
//     devirtualises completely EVEN WITH a facade in the same built pipeline.
//   HOST_consumer_unit   : blr=0 bl=0 (23 instrs). The accumulator walk devirts.
//   FACADE_morsel        : blr=1 bl=0 (38 instrs). Exactly ONE indirect call (the
//     opaque plugin ABI hop, `blr x8`), and it sits BETWEEN the morsel-loop guard
//     (`cbz x20`) and the morsel back-edge (`b.lo`), i.e. once per MORSEL, with NO
//     inner per-record loop around it.
//
// So §7-4b's premise holds: the host's per-record dispatch stays zero-blr while a
// facade is present, and the facade's only indirection is the per-morsel capability
// hop (the cdylib ABI seam), amortised across the morsel's records. Proven two
// ways (objdump + runtime call count). Leeway satisfied: the per-morsel-capability
// / sub-engine shape is proven (the plugin owns its cursor and processes "the next
// morsel of `len` records").
//
// FINDING for the build phase (mechanical, not a wall): the per-WU Context's
// BatchApi (and EachApi) hand the body a morsel-RELATIVE range (`start` always 0,
// `len` = morsel length); the engine adds `morsel.start` internally inside
// reader()/writer(). A facade that hands an EXTERNAL plugin a host column SLICE by
// ABSOLUTE index therefore needs a morsel-absolute accessor the Context does not
// yet expose (e.g. `ctx.morsel_range()` returning the absolute [start, start+len),
// or `ctx.read_slice::<T>()` / `write_slice::<T>()` returning the morsel's `&[T]` /
