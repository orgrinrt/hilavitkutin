//! P1 R3c: the throughput axis numerator (`record_count`) is exposed through the
//! engine-owned `MetaBlock` so an `OnMeta<ScheduleEnd>` hook can read it and
//! divide by `ema_pass_duration_ns` for the throughput trend. The engine sets
//! `SchedulerMetrics::last_record_count` beside the pass-duration EMA fold in
//! every run path. This test registers a known record count, runs a frame, and
//! asserts the hook observed it through the bridge. Mirrors
//! `gate2_meta_metrics.rs` (which proves the same bridge for `pass_count`).
//!
...
// OnMeta<ScheduleEnd> hook: read the throughput numerator through the bridge.
struct EndWu;
...
impl WorkUnit<OnMeta<ScheduleEnd>> for EndWu {
    ...
    fn execute<'frame>(&self, ctx: &Self::Ctx<'frame>) {
        let rc = ctx.meta::<SchedulerMetrics>().last_record_count.get();
        // SAFETY: Mark reserved (CAP); one append per frame stays in capacity.
        unsafe { ctx.accums().append::<Mark, _>(Mark(rc.0 as u32)) };
    }
}

#[test]
fn end_hook_reads_record_count_through_the_bridge() {
    ...
    // The engine sets last_record_count at frame end (beside the EMA fold), so a
    // hook observes frame N-1's value, like the pass-duration EMA. Frame 1's hook
    // sees the seed (0); frame 2's hook sees frame 1's record count.
    ...
    assert_eq!(buf1, [9, 0], "frame 1: end hook reads the seed record count (0)");
    ...
    assert_eq!(
        buf2,
        [9, RECORDS as u32],
        "frame 2: end hook reads frame 1's record count through the bridge",
    );
}
