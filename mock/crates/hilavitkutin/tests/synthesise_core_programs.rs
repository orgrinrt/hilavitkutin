...
use arvo_tensor::Dim;
use hilavitkutin::plan::core_program::synthesise_core_programs;
use hilavitkutin::plan::{PlanDims, PlanInputs, compute_execution_plan};
...
fn empty_plan_yields_empty_per_core_programs() {
    ...
    let programs = synthesise_core_programs::<TestDims, PhasesPerCore, TrunksPerCore, FibersPerCore>(
        &plan,
    ...
    for p in programs.as_ref().iter() {
        assert_eq!(p.phase_count, <USize as Identity<Additive>>::IDENTITY);
        assert_eq!(p.range_count, <USize as Identity<Additive>>::IDENTITY);
    }
}
...
fn single_unit_plan_assigns_one_core_one_fiber() {
    ...
    let programs = synthesise_core_programs::<TestDims, PhasesPerCore, TrunksPerCore, FibersPerCore>(
        &plan,
        USize(1),
    ); // lint:allow(no-bare-numeric) reason: single-core smoke; tracked: #427
    ...
    assert_eq!(
        c0.progress_slot_idx,
        <USize as Identity<Additive>>::IDENTITY
    );
    assert_eq!(
        c0.phase_arrived_offset,
        <USize as Identity<Additive>>::IDENTITY
    );
...
fn multi_fiber_plan_distributes_across_cores() {
    ...
    let programs = synthesise_core_programs::<TestDims, PhasesPerCore, TrunksPerCore, FibersPerCore>(
        &plan,
        USize(2),
    ); // lint:allow(no-bare-numeric) reason: two-core smoke; tracked: #427
    ...
    assert!(
        c1_base >= c0_base + c0_count,
        "progress slot ranges must not overlap"
    ); // lint:allow(no-bare-numeric) reason: invariant; tracked: #427

    // phase_arrived_offset is sequential per core (0, 1).
    assert_eq!(
        progs[0].phase_arrived_offset,
        <USize as Identity<Additive>>::IDENTITY
    );
