...
struct Member<Full, Stores, GW, CU, CS, Adj, Pos, const TRUNK: usize>(PhantomData<(Full, Stores, GW, CU, CS, Adj, Pos)>); // lint:allow(no-bare-numeric) reason: const-generic trunk carrier; tracked: #121
...
impl<
    ...
    Pos: WitnessIndex,
    const TRUNK: usize, // lint:allow(no-bare-numeric) reason: const-generic trunk carrier; tracked: #121
