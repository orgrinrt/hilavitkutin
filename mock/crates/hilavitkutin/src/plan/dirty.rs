...
impl<C: Capacity> DirtyMask<C> {
    ...
    const _ASSERT_FITS_IN_USIZE: () = assert!(
        cap_size(C::CAP) <= 64, // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: const-context size assertion; tracked: #429
    ...
    pub const fn empty() -> Self {
        Self::_ASSERT_FITS_IN_USIZE;
        Self {
            bits: <USize as Identity<Additive>>::IDENTITY,
            _cap: PhantomData,
        }
    }
