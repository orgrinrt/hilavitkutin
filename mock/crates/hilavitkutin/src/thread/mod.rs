...
//!
//! `assign_cores` maps trunks round-robin onto cores and
//! `classify_cores` asks the platform which cores are which.
//! Work-stealing is not part of v1; it arrives with v2 as
//! `Executor::try_steal` behind a `work-stealing` feature.
...
pub use parking::{
    ...
    atomic_wake_all,
    pick_tier,
    predicted_wait_ns_load,
    predicted_wait_ns_store,
    spin,
    spin_budget_for,
};
...
pub fn assign_cores<D: PlanDims>(
    ...
    let mut max_trunks_needed: usize = 0; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: loop accumulator; tracked: #72
    let phase_count = plan.phase_count.0;
    let phases = plan.phases.as_ref();
    let mut p: usize = 0; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: loop index; tracked: #72
    while p < phase_count {
    ...
    // CoreAssignment::new().
    let mut i: usize = 0; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: loop index; tracked: #72
    while i < width {
...
mod assign_cores_tests {
    ...
    fn single_phase_single_trunk_4_cores() {
        ...
        let mut i: usize = 1; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: loop start; tracked: #72
        while i < LANES {
            assert_eq!(trunk_index[i], NO_TRUNK);
            i += 1; // lint:allow(no-bare-numeric) reason: loop increment; tracked: #72
        }
    ...
    #[test]
    fn width_clamps_to_the_core_count() {
        // Five trunks wanted, two cores present: the core count is the bound.
        let plan = plan_with_phase_trunks(&[5]); // lint:allow(no-bare-numeric) reason: fixture trunk count; tracked: #72
        let cores = USize(2); // lint:allow(no-bare-numeric) reason: test core count; tracked: #72
        let result = assign_cores(cores, &plan);
        assert_eq!(result.assigned_count, cores);
        let trunk_index = result.trunk_index.as_ref();
        let mut i: usize = 0; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: loop index; tracked: #72
        while i < LANES {
            let want = if i < cores.0 { USize(i) } else { NO_TRUNK };
            assert_eq!(trunk_index[i], want);
            i += 1; // lint:allow(no-bare-numeric) reason: loop increment; tracked: #72
        }
    }

    #[test]
    fn width_clamps_to_the_lane_capacity() {
        // Twelve trunks and sixteen cores, but only `LANES` (8) slots exist:
        // the lane capacity is the bound, and every slot is filled.
        let plan = plan_with_phase_trunks(&[12]); // lint:allow(no-bare-numeric) reason: fixture trunk count past the lane capacity; tracked: #72
        let cores = USize(16); // lint:allow(no-bare-numeric) reason: test core count past the lane capacity; tracked: #72
        let result = assign_cores(cores, &plan);
        assert_eq!(result.assigned_count, USize(LANES));
        let trunk_index = result.trunk_index.as_ref();
        let mut i: usize = 0; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: loop index; tracked: #72
        while i < LANES {
            assert_eq!(trunk_index[i], USize(i));
            i += 1; // lint:allow(no-bare-numeric) reason: loop increment; tracked: #72
        }
    }
    ...
    fn three_phases_max_3_trunks_4_cores() {
        ...
        let mut i: usize = 3; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: loop start; tracked: #72
        while i < LANES {
            assert_eq!(trunk_index[i], NO_TRUNK);
            i += 1; // lint:allow(no-bare-numeric) reason: loop increment; tracked: #72
        }
