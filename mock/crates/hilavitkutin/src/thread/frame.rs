...
pub fn frame_publish<'arena, const C: usize, const P: usize>(
...
pub fn frame_await<'arena, const C: usize, const P: usize>(
    // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: const-generic array size; rust grammar requires usize; tracked: #121
    pool: &PoolFrame<'arena, C, P>,
    last_seen: USize,
    spin_budget: USize,
) -> USize {
    // Bounded middle-tier pre-roll (Topic 6 axis K): re-load and
    // re-check up to the budget before the first park. Each iteration
    // observes the word, so a bump during the spin returns without
    // ever sleeping; the eventual park keeps the load-check-wait
    // pattern below, so the lost-wakeup discipline is untouched. A
    // zero budget is park-immediately exactly.
    let mut spins = USize(0); // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: spin counter init; tracked: #121
    loop {
        let cur = pool.seq.load(Ordering::Acquire);
        if cur as usize != last_seen.0 {
            // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: atomic u32 widened to compare against USize; tracked: #121
        ...
        if spins.0 < spin_budget.0 {
            spins = USize(spins.0 + 1); // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: const-arith on USize internal; tracked: #72
            core::hint::spin_loop();
            continue;
        }
...
pub fn frame_done_arrive<'arena, const C: usize, const P: usize>(
    // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: const-generic array size; rust grammar requires usize; tracked: #121
    pool: &PoolFrame<'arena, C, P>,
    expected: USize,
) {
...
pub fn frame_await_done<'arena, const C: usize, const P: usize>(
    // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: const-generic array size; rust grammar requires usize; tracked: #121
    pool: &PoolFrame<'arena, C, P>,
    expected: USize,
    spin_budget: USize,
) {
    // Same bounded pre-roll as `frame_await`: frame completion is
    // usually imminent when the scheduler arrives here, which is the
    // regime the spin tier exists for.
    let mut spins = USize(0); // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: spin counter init; tracked: #121
    loop {
        let d = pool.done.load(Ordering::Acquire);
        if (d as usize) == expected.0 {
            // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: atomic u32 widened to compare against USize; tracked: #121
            return;
        }
        if spins.0 < spin_budget.0 {
...
pub fn request_shutdown<'arena, const C: usize, const P: usize>(
    // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: const-generic array size; rust grammar requires usize; tracked: #121
    pool: &PoolFrame<'arena, C, P>,
) {
...
pub fn frame_exit_arrive<'arena, const C: usize, const P: usize>(
    // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: const-generic array size; rust grammar requires usize; tracked: #121
    pool: &PoolFrame<'arena, C, P>,
    expected: USize,
) {
...
/// mainloop. Deliberately park-only (no spin tier): shutdown runs once
/// per pool lifetime and its latency is irrelevant. After this returns, the `PoolFrame` and the carrier the workers
/// read are safe to drop. This is the shutdown join (no thread join needed).
pub fn await_exit<'arena, const C: usize, const P: usize>(
    // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: const-generic array size; rust grammar requires usize; tracked: #121
    pool: &PoolFrame<'arena, C, P>,
    expected: USize,
) {
