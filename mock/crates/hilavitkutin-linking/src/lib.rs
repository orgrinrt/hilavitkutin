...
//! loader. Extension descriptor semantics, lifecycle contracts, and
//! capability dispatch live one layer up in `hilavitkutin-extensions`.
//!
...
//! lifecycle is strictly per-library.

mod backend;
mod error;
mod symbol;

pub use error::{LinkError, IncompatibilityError};
pub use symbol::{LibrarySymbol, StaticRef, Symbol};
...
///
/// RAII: dropping the `Library` closes the library on the OS side.
/// `Library::close` is the explicit form that returns the OS close
/// result, for consumers that want to surface unload errors instead
/// of dropping them silently.
///
/// Every `Symbol<'_, T>` or `StaticRef<'_, T>` resolved from an
/// `Library` borrows it, so the library remains loaded for the
...
impl Library {
    ...
    ///
    /// `T` must satisfy the sealed `LibrarySymbol` marker, namely
    ...
    /// `&T`. Use for the typical pattern where an extension exports
    /// a `#[no_mangle] pub static EXT_DESCRIPTOR: ExtDescriptor = ...;`.
    /// The resolved symbol is the address of the static, not a
    ...
    pub fn close(self) -> Outcome<(), LinkError> {
...
/// the caller has previously resolved from a loaded library.
///
