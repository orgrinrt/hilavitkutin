//! Variant v3_shapebound: builds its storage layout as untimed setup (statements before
//! the `run` block), runs the morsel loop in the timed `run` block. See
//! `rsb_kernels::v3` for the layout + kernel; this cdylib is the thin
...
#[bench_variant("v3_shapebound", sizes = [256, 1024, 4096, 16384, 65536, 262144, 1048576])]
fn run_v3_shapebound<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    // Untimed setup: build the storage layout + columns from the seeded input.
    let state = v3::build(input);
    let r = timed! {
        run {
