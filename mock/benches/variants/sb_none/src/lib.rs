//! Variant: no-swap baseline: the timed frame is the all-clean skip frame, the floor the two swap arms are read against.
//!
...
#[bench_variant("sb_none", sizes = [64, 1024, 8192])]
fn run_sb_none<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        STATE.with(|s| {
            let mut s = s.borrow_mut();
            if s.as_ref().map(|(n, _)| *n) != Some(N) {
                *s = Some((N, prepare_band(SwapMode::None, seed_from_input(input), N * 1024)));
            }
        });
        setup {
