//! Variant: 1 KiB payload: the timed body is one replace_value install of a 256-word value.
//!
...
use swap_common::prepare_cost_1k;
...
#[bench_variant("sc_v1k", sizes = [64])]
fn run_sc_v1k<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        STATE.with(|s| {
            let mut s = s.borrow_mut();
            if s.is_none() {
                *s = Some(prepare_cost_1k(seed_from_input(input)));
            }
        });
        setup {
