//! Variant: value-swap arm: an untimed replace_value precedes each timed frame, so the frame pays the Replaceable dirty cone (producer plus consumer) and nothing else.
//!
...
#[bench_variant("sb_value", sizes = [64, 1024, 8192])]
fn run_sb_value<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        STATE.with(|s| {
            let mut s = s.borrow_mut();
            if s.as_ref().map(|(n, _)| *n) != Some(N) {
                *s = Some((N, prepare_band(SwapMode::Value, seed_from_input(input), N * 1024)));
            }
        });
        setup {
