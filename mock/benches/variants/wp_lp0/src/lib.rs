//! Variant: local-wait control at budget 0: identical park-immediately protocol through the parameterised wait, validating the local harness against wp_park.
//!
...
#[bench_variant("wp_lp0", sizes = [64, 1024, 8192])]
fn run_wp_lp0<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        STATE.with(|s| {
            let mut s = s.borrow_mut();
            if s.is_none() {
                *s = Some(WakeCarrier::new_with(WaitPolicy::Local { budget: 0, hint: false }));
            }
        });
