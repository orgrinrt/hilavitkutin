//! Variant: greedy nearest-neighbour serpentine 0,1,2,3,7,6,5,4,8,9,10,11: maximises immediate consecutive sharing (bandwidth 7, total 41, 11/11 consecutive-adjacent).
//!
...
#[bench_variant("riv_snake", sizes = [64, 256, 1024, 2048, 4096, 8192, 16384])]
fn run_riv_snake<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        STATE.with(|s| {
            let mut s = s.borrow_mut();
            if s.as_ref().map(|(n, _)| *n) != Some(N) {
                *s = Some((N, prepare(GridOrder::Snake, seed_from_input(input), N * 1024)));
            }
        });
        run {
