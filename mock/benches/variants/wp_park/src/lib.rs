//! Variant: park-immediately baseline: budget 0 parks on the first miss, the pre-round shipped shape.
//!
...
#[bench_variant("wp_park", sizes = [64, 1024, 8192])]
fn run_wp_park<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        STATE.with(|s| {
            let mut s = s.borrow_mut();
            if s.is_none() {
                *s = Some(WakeCarrier::new_with(WaitPolicy::Shipped { budget: 0 }));
            }
        });
