//! Variant: no-hint spin, budget 2048 plain re-loads (about 0.8 us window).
//!
...
#[bench_variant("wp_nh2k", sizes = [64, 1024, 8192])]
fn run_wp_nh2k<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        STATE.with(|s| {
            let mut s = s.borrow_mut();
            if s.is_none() {
                *s = Some(WakeCarrier::new_with(WaitPolicy::Local { budget: 2048, hint: false }));
            }
        });
